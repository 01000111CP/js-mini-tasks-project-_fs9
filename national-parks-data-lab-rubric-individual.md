...
- [X] **Create an array** - Define an array of at least 6 park names as strings
      using array literal notation (square brackets)
- [X] **Access by index** - Use bracket notation to access the first item (index 0),
      the last item (using .length - 1), and a middle item
- [X] **Display array results** - Use the provided show() function to display
      array values on the page

### Arrays - Looping

- [X] **Standard for loop** - Loop through an array using a for loop with an
      index counter variable and console.log() each item
- [X] **for...of loop** - Loop through an array using a for...of loop and
      console.log() each item
- [X] **forEach method** - Loop through an array using .forEach() with a
      callback function and console.log() each item

### Arrays - Transformation Methods

- [X] **map() method** - Use .map() to create a new array by transforming each
      element (e.g., adding " National Park" to each name). Store and display result.
- [X] **filter() method** - Use .filter() to create a new array containing only
      elements matching a condition. Store and display result.
- [X] **find() method** - Use .find() to locate the first element matching a
      condition. Store and display result.

### Objects - Creation and Properties

- [X] **Empty object with dynamic properties** - Create an empty object using {}
      and add at least 5 properties one at a time using dot notation
- [X] **Object literal** - Create an object using object literal notation with at
      least 4 properties plus a space-key property and a nested object
- [X] **Dot notation access** - Access at least two object properties using dot
      notation and display results
- [X] **Bracket notation access** - Access at least two object properties using
      bracket notation, including accessing via a variable as the key
- [X] **Space-key property** - Create a property with a space in its name and use
      bracket notation (the only way) to access it
- [X] **Delete a property** - Remove a property using the delete operator and show
      it is gone (undefined) with show()

### Objects - Nested Data and Chaining

- [X] **Nested object** - Create an object that contains another object as a property
- [X] **Access nested property - longhand** - Access a nested value using intermediate
      variables (storing the inner object first, then accessing its property)
- [X] **Access nested property - shorthand** - Access the same value using chained
      dot notation (object.inner.property)

### Objects - Methods

- [X] **Add a method** - Add a function as a property on an existing object using
      assignment (myObj.methodName = function() { ... })
- [X] **Call a method** - Call the method and display the result with show()
- [X] **Use this in a method** - Create an object with a method that uses the this
      keyword to reference at least two of the object's own properties
- [X] **function keyword for methods** - Methods that use this are written with the
      function keyword, not as arrow functions

### Arrays of Objects - Combined

- [X] **Array of objects** - Create an array where each element is a park object
      with the same set of properties (at least 5 objects, 6 properties each)
- [X] **Display as table** - Use showTable() to display the array of objects
- [X] **Loop through array of objects** - Use .forEach() to log a formatted string
      for each object using console.log()
- [X] **filter() on array of objects** - Create at least one filtered subset of
      the parks array using a property condition (e.g., entranceFee === 0)
- [X] **map() on array of objects** - Create an array of strings from the parks
      array using .map() and display the result with show()

### Object Manipulation

- [X] **Object.assign() copy** - Copy an object using Object.assign({}, source),
      modify a property on the copy, and show both to prove the original is unchanged
- [X] **Primitive value copy** - Demonstrate that changing a copied primitive does
      not affect the original (numbers, strings)
- [X] **Object reference behavior** - Demonstrate that assigning an object to a new
      variable gives both variables the same reference (changing one changes both)
- [X] **Object === comparison** - Show that two separate objects with identical
      contents return false with === and explain why
- [X] **const mutation** - Show that properties on a const object can be changed
      (mutation is allowed)
- [X] **const reassignment** - Show a commented-out reassignment attempt with a
      comment explaining the TypeError it would throw
- [X] **const array mutation** - Show that .push() works on a const array but
